...
def parse_requirement(requirement: str) -> Tuple[float, float, float]:
    """Extract pax, hours/day and days from strings like '4 pax/8h/2days'."""
    req = clean_text(requirement).lower()
    pax = hours = days = np.nan
...
def is_assigned_name(name: str) -> bool:
    ...
    return value not in UNASSIGNED_NAMES and not value.startswith("TBC")
...
def find_header_row(rows: List[List[str]]) -> int:
    ...
    if best_score < 5:
        raise ValueError("Could not detect the header row. Please upload the table with the year row and month row intact.")
    return best_row
...
def detect_columns(rows: List[List[str]], header_idx: int):
    ...
    # Fixed left-side structure based on your table format:
    # Col A = SN / Project, Col B = Activity or Role, Col C = Name.
    sn_col = 0
    role_col = 1
    name_col = 2
    ...
    for idx in range(len(header)):
        ...
        if idx in month_cols:
            ...
            year = current_year
            label = f"Unknown-{clean_text(header[idx])}-{idx}" if year is None else pd.Timestamp(year, month_num, 1).strftime("%Y-%b")
            month_dt = pd.NaT if year is None else pd.Timestamp(year, month_num, 1)
            month_map.append({"col": idx, "label": label, "date": month_dt})
...
def is_project_header(sn: str, role: str, name: str, requirement: str, has_load: bool) -> bool:
    ...
    return sn_clean in PROJECT_HEADERS and clean_text(requirement) == "" and not has_load
...
def parse_resource_table(rows: List[List[str]]):
    ...
    sn_col, role_col, name_col, req_col, pax_col, hours_col, days_col, month_map = detect_columns(rows, header_idx)
    ...
    for source_row_no, row in enumerate(data_rows, start=header_idx + 2):
        ...
        month_values = {m["label"]: to_number(row[m["col"]]) for m in month_map if m["col"] < len(row)}
        ...
        if is_project_header(sn, role_or_activity, name, requirement, has_load):
            ...
            current_activity_no = ""
            current_requirement = ""
            current_pax = current_hours = current_days = np.nan
            continue

        # Skip the project display header row like: JRL | | Name
        ...
        # Rows with a requirement in Column D and NO person name are subtotal/activity rows.
        # Rows with a requirement AND a person name, like "T&C Manager | Eric Tan | 1 pax/8h/5 days",
        # are treated as actual resource rows and included in the load calculation.
        # Also treat rows with only an activity name and no load/name as activity headers.
        is_activity_row = (bool(requirement) and not name) or (role_or_activity and not name and not has_load)
        if is_activity_row:
        ...
        # Person/resource row. This may have a real name, TBC, or blank name.
...
def pivot_load(df: pd.DataFrame, index_col: str, value_col: str = "Load") -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    pivot = df.pivot_table(index=index_col, columns="Month", values=value_col, aggfunc="sum", fill_value=0)
...
def create_excel_export(
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        overload_format = workbook.add_format({"bg_color": "#FFD6D6"})
        ...
        for sheet_name, df in {
            ...
            for col_num, col_name in enumerate(df.columns):
                if col_name in {"Load", "Overload", "Shortage Load", "Available Pax"} or re.match(r"^\d{4}-[A-Za-z]{3}$", str(col_name)):
            ...
            if sheet_name in {"Person_Load", "Person_Overload"} and "Overload" in df.columns:
                ...
                worksheet.conditional_format(1, over_col, max(len(df), 1), over_col, {"type": "cell", "criteria": ">", "value": 0, "format": overload_format})
            if sheet_name == "Role_Hiring_Gap" and "Extra Pax Needed" in df.columns:
                extra_col = df.columns.get_loc("Extra Pax Needed")
                worksheet.conditional_format(1, extra_col, max(len(df), 1), extra_col, {"type": "cell", "criteria": ">", "value": 0, "format": overload_format})
...
st.caption("Upload your resource planning table. The app treats 1.0 load as one full person-month by default.")

with st.sidebar:
    st.header("Settings")
    capacity = st.number_input("Monthly capacity per person", min_value=0.1, max_value=5.0, value=1.0, step=0.1)
    group_col = st.radio("Hiring gap grouping", ["Role Group", "Role"], index=0)
    show_zero_rows = st.checkbox("Show zero-load resource rows", value=False)

uploaded_file = st.file_uploader("Upload resource planning table", type=["txt", "csv", "xlsx", "xls"])

if uploaded_file is None:
    st.info("Upload the pasted table as .txt/.csv or Excel. The format can follow your current table with Year row, Month row, Requirement, Pax, Hours/day, Days, and monthly load columns.")
    st.stop()
...
if detail_df.empty or long_df.empty:
    st.warning("The table was read, but no resource-load rows were detected. Check that the detail rows are below the activity requirement rows and that monthly cells contain numeric loads.")
    st.stop()
...
monthly_total = long_df.groupby(["Month", "Month Date"], as_index=False).agg(Total_Load=("Load", "sum"))
monthly_extra = role_gap.groupby(["Month", "Month Date"], as_index=False).agg(Total_Extra_Pax_Needed=("Extra Pax Needed", "sum"))
monthly_summary = monthly_total.merge(monthly_extra, on=["Month", "Month Date"], how="outer").fillna(0).sort_values("Month Date")

person_overload = person_load[person_load["Overload"] > 0].copy() if not person_load.empty else pd.DataFrame()

peak_total_load = monthly_summary["Total_Load"].max() if not monthly_summary.empty else 0
peak_month = monthly_summary.loc[monthly_summary["Total_Load"].idxmax(), "Month"] if not monthly_summary.empty else "-"
max_person_load = person_load["Load"].max() if not person_load.empty else 0
worst_person = person_load.loc[person_load["Load"].idxmax(), "Name"] if not person_load.empty else "-"
peak_extra = monthly_summary["Total_Extra_Pax_Needed"].max() if not monthly_summary.empty else 0
peak_extra_month = monthly_summary.loc[monthly_summary["Total_Extra_Pax_Needed"].idxmax(), "Month"] if not monthly_summary.empty else "-"
...
tab_dashboard, tab_clean, tab_people, tab_hiring, tab_export = st.tabs(
    ["Dashboard", "Cleaned Rows", "Person Overload", "Hiring Gap", "Export"]
)

with tab_dashboard:
    st.subheader("Monthly Demand")
    chart_df = monthly_summary.copy()
    chart_df["Month Text"] = chart_df["Month"]
    if len(chart_df) > 0:
        fig = px.line(chart_df, x="Month Text", y="Total_Load", markers=True, title="Total monthly load")
        fig.update_layout(xaxis_title="Month", yaxis_title="Load / Pax-month", xaxis_tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)

        fig2 = px.bar(chart_df, x="Month Text", y="Total_Extra_Pax_Needed", title="Total extra pax needed by month")
        fig2.update_layout(xaxis_title="Month", yaxis_title="Extra pax needed", xaxis_tickangle=-45)
    ...
    if hiring_summary.empty:
        ...
        st.dataframe(hiring_summary[[group_col, "Month", "Load", "Available Pax", "Shortage Load", "Extra Pax Needed", "Status"]], use_container_width=True)

with tab_clean:
    st.subheader("Cleaned Resource Rows")
    st.caption("Rows with Column D requirement are treated as activity/subtotal rows. Rows below them are treated as resource rows and inherit the activity requirement.")
    st.dataframe(detail_display, use_container_width=True, height=600)

with tab_people:
    st.subheader("Person Load")
    if person_load.empty:
        st.warning("No assigned names were found. TBC/blank rows are counted for role demand, but not for individual overload.")
    else:
        person_pivot = pivot_load(person_load, "Name")
        st.write("Cells above capacity are highlighted.")
        st.dataframe(style_overload_table(person_pivot, capacity), use_container_width=True, height=500)

        st.subheader("Overload Details")
        if person_overload.empty:
            st.success("No named person exceeds the monthly capacity.")
        else:
            st.dataframe(person_overload, use_container_width=True, height=500)

with tab_hiring:
    st.subheader(f"Hiring Gap by {group_col}")
    st.caption("Demand is total monthly load. Available pax comes from the editable headcount table in the sidebar. Extra pax = CEILING(MAX(Demand - Available, 0)).")

    role_pivot = pivot_load(role_gap, group_col, value_col="Extra Pax Needed")
    st.write("Extra pax needed by month")
    st.dataframe(role_pivot, use_container_width=True, height=500)

    st.write("Detailed gap calculation")
    st.dataframe(role_gap, use_container_width=True, height=500)

with tab_export:
    st.subheader("Download Results")
    export_bytes = create_excel_export(detail_df, person_load, role_gap, monthly_summary, hiring_summary)
    st.download_button(
    ...
    st.write("Cleaned_Input, Person_Load, Person_Overload, Role_Hiring_Gap, Hiring_Summary, Monthly_Summary")
